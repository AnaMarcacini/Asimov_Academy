...
def iterar_sobre_df_book(df_book_var: pd.DataFrame, ativos_org_var={}) -> dict:
    # df_book_var: pd.DataFram --> o parametro deve ser do tipo dataframe || dicionario vazio e retorna um dicionário
    for _, row in df_book_var.iterrows():# iterrows itera colunas # interra todos os elementos 
...
def atualizar_historical_data(df_historical_var: pd.DataFrame, ativos_org_var={}) -> pd.DataFrame:
    tv = TvDatafeed()
    for symb_dict in ativos_org_var.items():# ver cada tipo de ativos
        new_line = tv.get_hist(*symb_dict, n_bars=5000)[['symbol','close']].reset_index() # pegar historico da requisição || historico da ação    *symb_dict -> abre os valores do dicionario
        # n_bars=5000 - max de valores  
        # ['symbol' - valor da ação,'close ' valor de fechamento da ação] 
        #.reset_index() --> atualiza valore do indice dela 
        df_historical_var = pd.concat([df_historical_var, new_line], ignore_index=True) # adiciona a linha ao que já temos 

    return df_historical_var.drop_duplicates(ignore_index=True)# tira valores duplicados
