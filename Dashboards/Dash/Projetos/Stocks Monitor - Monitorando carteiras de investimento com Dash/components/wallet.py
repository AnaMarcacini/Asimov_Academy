...
def generate_card(info_do_ativo):
    new_card =  dbc.Row([
            dbc.Col([
                ...
                ], class_name=info_do_ativo['class_card'])# verifica se o cartão usa a formatação de venda (verde) ou de compra (vermelho)
            ])
...
def generate_list_of_cards(df):
    ...
    for row in df.index:
        ...
        #Seleciona a classe do card corretamente (card_compra ou card_venda)
        if infos['tipo'] == 'Compra':
            infos['class_card'] = 'card_compra'
        else:
            infos['class_card'] = 'card_venda'
        infos['id'] = row # id recebe as informações do card
...
layout = dbc.Container([
    dbc.Row([
        ...
        ], md=12, id='layout_wallet', style={"height": "100%", "maxHeight": "36rem", "overflow-y": "auto"}) #"overflow-y" - cria um  scroll -> evita reprimir tudo em um só
    ], className='g-2 my-auto')
],fluid=True),
...
def func_modal(n1, n2, book_data, event, ativo, open, radio, preco, periodo, vol):
    ...
    if trigg_id == '': # nenhum trigger - > inicializando dashboard
        df_book_data = df_book_data.sort_values(by='date', ascending=True)
    ...
    # 2. Salvando ativo
        ...
        if None in [ativo, preco, vol] and open:
            ...
            ativo = ativo.upper()
            if tv.search_symbol(ativo,'BMFBOVESPA'): # procura se o ativo existe na bolsa  BMFBOVESPA
                exchange = 'BMFBOVESPA'
                preco = round(preco, 2) # 2 casas decimais 
    ...
    if 'delete_event' in trigg_id:
        ...
        trigg_id = json.loads(trigg_id)#gera json
        df_book_data.drop([str(trigg_id['index'])], inplace=True)# remove de vdd(inplace) 
        df_book_data.reset_index(drop=True, inplace=True)
        df_book_data = df_book_data.sort_values(by='date', ascending=True)
        df_book_data.to_csv('book_data.csv')
        book_data = df_book_data.to_dict()
